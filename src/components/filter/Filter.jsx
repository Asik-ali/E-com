...
function Filter() {
  ...
  useEffect(() => {
    ...
    const categorySet = new Set(product.map(item => item.category));
    // Convert Set to array and set it in the state
    setUniqueCategories(Array.from(categorySet));

    // Create a Set to store unique prices
    const priceSet = new Set(product.map(item => item.price));
    // Convert Set to array and set it in the state
    setUniquePrices(Array.from(priceSet));
  }, [product]);
