                ...
                <div className="flex flex-wrap -m-4">
                    {product.filter((obj)=> obj.title.toLowerCase().includes(searchkey))
                     ...
                     .filter((obj) => obj.price.includes(filterPrice)).slice(0,8).map((item, index) => {
                        ...
                        return (
                            <div    key={index} className="p-4 md:w-1/3 md:p-2 lg:w-1/4  drop-shadow-lg " >
                                <div  className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                    ...
                                    <div className="p-5 border-t-2 ">
                                        <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1" style={{ color: mode === 'dark' ? 'white' : '', }}>RIO</h2>
                                        <h1 className="title-font text-lg font-medium text-gray-900 mb-5 h-10" style={{ color: mode === 'dark' ? 'white' : '', }}>{title}</h1>
                                        {/* <p className="leading-relaxed mb-3">{item.description}</p> */}
                                        <p className="leading-relaxed mb-5" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{price}</p>
